...
import { getNearbyStations as getSecureNearbyStations } from '../services/api'; // 👈 Renamed import
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    loadGarages();
    getUserLocation(); // 👈 Changed function name
    return () => unsubscribe();
  }, []);
  ...
  // 👈 RENAMED: This gets the user's GPS location
  ...
  // 👈 UPDATED: Uses the renamed import
  const fetchStations = async (lat, lng) => {
    ...
    try {
      ...
      console.log("🔍 Found", stations.length, "garages via secure backend");
      setPlaces(stations);
      await syncGaragesWithDB(stations);
              ...
              <View key={place.place_id || i} style={{
                ...
                <View style={{ marginBottom: 12 }}>
                  ...
                  {p?.previousPrice && p.previousPrice > 0 && (
                    <Text style={{ color: '#64748b', fontSize: 10, marginTop: 2 }}>
                      Previous: R{p.previousPrice.toFixed(2)} ({p.previousUpdater || 'Unknown'})
                    </Text>
                  )}
                </View>
