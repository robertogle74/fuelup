          ...
          onPress: async () => {
            ...
            try {
              ...
              for (const garage of garages) {
                try {
                  ...
                  if (!region) {
                    ...
                    // Save region to garage for future updates
                  ...
                  const dieselDoc = await getDoc(doc(db, 'fuel', dieselId));
                  const petrolDoc = await getDoc(doc(db, 'fuel', petrolId));
                  ...
                  if (dieselDoc.exists()) {
                    ...
                    const currentTimestamp = dieselDoc.data().timestamp;
                      ...
                      await setDoc(doc(db, 'fuel_test', dieselId), {
                      //await setDoc(doc(db, 'fuel', dieselId), {
                      price: dieselPrice,
                  ...
                  if (petrolDoc.exists()) {
                    ...
                    await setDoc(doc(db, 'fuel_test', petrolId), {
                      //await setDoc(doc(db, 'fuel', petrolId), {
                      price: petrolPrice,
                      previousPrice: currentPrice,
                      previousUpdater: currentUpdater,
                      previousUpdateDate: currentTimestamp,
              ...
              Alert.alert('Update Complete', 
                ...
                `❌ Errors: ${errors}\n\n` +
                `Previous prices have been saved.`
              );
            } catch (error) {
